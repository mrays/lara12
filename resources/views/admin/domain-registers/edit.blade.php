...
@section('title', 'Edit Register Domain')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card mb-4">
                <div class="card-body">
                    <h5 class="mb-0">
                        <i class="bx bx-edit me-2"></i>Edit Register Domain
                    </h5>
                    <p class="text-muted mb-0">Ubah informasi register domain: {{ $register->name }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.domain-registers.update', $register) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('name') is-invalid @enderror" 
                                   id="name" name="name" value="{{ old('name', $register->name) }}" required>
                            @error('name')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('username') is-invalid @enderror" 
                                   id="username" name="username" value="{{ old('username', $register->username) }}" required>
                            @error('username')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="url" class="form-control @error('login_link') is-invalid @enderror" 
                                   id="login_link" name="login_link" value="{{ old('login_link', $register->login_link) }}" required
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('expired_date') is-invalid @enderror" 
                                   id="expired_date" name="expired_date" value="{{ old('expired_date', $register->expired_date->format('Y-m-d')) }}" required>
                            @error('expired_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                <option value="active" {{ old('status', $register->status) == 'active' ? 'selected' : '' }}>Active</option>
                                <option value="suspended" {{ old('status', $register->status) == 'suspended' ? 'selected' : '' }}>Suspended</option>
                                <option value="expired" {{ old('status', $register->status) == 'expired' ? 'selected' : '' }}>Expired</option>
                            </select>
                            @error('status')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('notes') is-invalid @enderror" 
                                      id="notes" name="notes" rows="3">{{ old('notes', $register->notes) }}</textarea>
                            @error('notes')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="bx bx-save me-1"></i>Update Register
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <div class="col-md-4">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">
                        <i class="bx bx-info-circle me-2"></i>Informasi Register
                    </h5>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label class="form-label">ID Register:</label>
                        <p class="mb-0"><strong>#{{ $register->id }}</strong></p>
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label">Dibuat:</label>
                        <p class="mb-0">{{ $register->created_at->format('M d, Y H:i') }}</p>
                    </div>
                    
                    <div class="mb-3">
                        <label class="form-label">Terakhir Update:</label>
                        <p class="mb-0">{{ $register->updated_at->format('M d, Y H:i') }}</p>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <p class="mb-0">
                            <span class="badge bg-info">{{ $register->clients->count() }} Client</span>
                        </p>
                    </div>
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button type="button" class="btn btn-sm btn-outline-secondary" onclick="toggleStatus({{ $register->id }})">
...
function showCurrentPassword() {
    fetch(`/admin/domain-registers/{{ $register->id }}/password`)
...
function showClients() {
    window.location.href = '{{ route("admin.client-data.index") }}?domain_register_id={{ $register->id }}';
}

// Check expiration
function checkExpiration() {
    const expiredDate = new Date('{{ $register->expired_date->format("Y-m-d") }}');
    ...
    let message = '';
    if (diffDays < 0) {
        message = `Register expired ${Math.abs(diffDays)} days ago!`;
    } else if (diffDays <= 30) {
        message = `Register will expire in ${diffDays} days!`;
    } else {
        message = `Register is valid for ${diffDays} more days.`;
    }
...
// Test login link
function testLogin() {
    window.open('{{ $register->login_link }}', '_blank');
}
